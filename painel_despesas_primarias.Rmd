...
renderPlotly({
  ...
  if (input$freeScale){
    graph<-
      graph+ facet_wrap(NO_FUNCAO_PT~., scales = "free_y", ncol = 4)
...
renderPlotly({
  ...
  graph<-
    ...
    mutate(conta = forcats::fct_reorder(CATEGORIA_RTN,total_paga_ano,.fun = sum, .desc = TRUE),
           funcao = str_wrap(NO_FUNCAO_PT,10),
           ano = ID_ANO) %>%  
    ...
    ungroup() %>%
    ggplot() +
    geom_line(aes(x=ano, y=despesa, color=funcao, text = conta))+
    scale_color_discrete_qualitative(palette = "Dark 3")+
    theme_light() +
    theme(
      axis.text.x = element_text(angle = 90),
  ...
  ggplotly(graph, tooltip = c("ano", "despesa", "funcao", "text"))
...
checkboxInput("valor_historico_conta","Usar valor historico")
...
renderPlotly({
  ...
  var_valor<- selecionaValor(input$valor_historico_conta)
  ...
  graph<-
    ...
    mutate(conta = forcats::fct_reorder(CATEGORIA_RTN,total_paga_ano,.fun = sum, .desc = TRUE),
           orgao = str_wrap(ORGAO_DESCRICAO,10),
           ano = ID_ANO) %>%  
    ...
    ungroup() %>%
    ggplot() +
    geom_line(aes(x=ano, y=despesa, color= orgao, text= conta))+
    scale_color_discrete_qualitative(palette = "Dark 3")+
    theme_light() +
    theme(
      axis.text.x = element_text(angle = 90),
  ...
  ggplotly(graph, tooltip = c("ano", "despesa", "orgao", "text"))
...
renderPlotly({
  ...
  graph<-
    ...
    mutate(conta = forcats::fct_reorder(CATEGORIA_RTN,total_paga_ano,.fun = sum, .desc = TRUE),
           ano = ID_ANO,
           programa = str_wrap(NO_PROGRAMA_PT,10)) %>%  
    ...
    ungroup() %>%
    ggplot() +
    geom_line(aes(x=ano, y=despesa, color=programa))+
    scale_color_discrete_qualitative(palette = "Dark 3")+
    theme_light() +
    theme(
      axis.text.x = element_text(angle = 90),
  ...
  ggplotly(graph, tooltip = c("ano", "programa",  "text"))
...
renderPlot({
  ...
  group_by(ID_ANO,NO_ACAO, CATEGORIA_RTN) %>%
  summarise(
    total_paga_ano = sum(total_paga_ano)
  ) %>%
  ungroup() %>%
  ggplot() +
  geom_line(aes(x=ID_ANO, y=total_paga_ano/10^6, color=NO_ACAO))+
    scale_color_discrete_qualitative(palette = "Dark 3")+
    theme_light() +
    theme(
      axis.text.x = element_text(angle = 90),
  ...
  if (input$freeScale_acao){
    graph<-
      graph+ facet_wrap(CATEGORIA_RTN~., scales = "free_y")

  } else{
    graph<-
      graph+ facet_wrap(CATEGORIA_RTN~.)
